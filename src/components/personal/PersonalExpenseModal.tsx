...
type Initial = { id: number; categoryId: number; amount: number; note: string | null; recurring: boolean };
  ...
  useEffect(() => {
    if (state.n > prevN.current) {
      ...
      if (state.ok) {
        ...
        toast(state.error ?? "Couldn't save", "error");
      }
    }
              ...
              <div className="space-y-4 px-5 py-4">
                ...
                <div>
                  ...
                  <select
                    name="categoryId"
                    required
                    defaultValue={isEdit ? String(initial!.categoryId) : ""}
                ...
                <label className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-700">
                  ...
                  Repeats every month
                  <span className="text-xs text-slate-400">(rent, subscriptions…)</span>
                </label>
              </div>

              <div className="flex justify-end gap-2 border-t border-slate-100 px-5 py-3">
                <button type="button" onClick={() => setOpen(false)} className="rounded-md px-3 py-2 text-sm text-slate-500 hover:bg-slate-100">
                  Cancel
                </button>
                <button
                  type="submit"
